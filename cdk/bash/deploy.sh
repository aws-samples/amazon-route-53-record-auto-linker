...
function bucket() {
  if [ -z $bucket ]; then
    bucket=$(aws ssm get-parameters --names "/r53linker/bucket" --profile ${PROFILE} | jq -r .Parameters[0].Value)
  fi
...
function deploy_lb_records() {
  if [ $component == "all" ] || [ $component == "lb.records" ]; then
    run "cdk deploy R53-Records --require-approval never --profile ${PROFILE}"
  fi
}
...
function destroy() {
  if [ $component == "all" ]; then
    ...
    run "cdk destroy R53-Records               --force --profile ${PROFILE}"
  fi
}
